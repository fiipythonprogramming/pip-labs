...
def exercise3():
    ...
    list of differences as follows: (Attention, dictionaries must be
    ...
    def solution1(elem1, elem2):
        ...
        if isinstance(elem1, dict):
            for key, value in elem1.items():
                ...
                values_checker = solution1(elem1[key], elem2[key])
                if not values_checker:
                    return False

        return True

    print(solution1(element1, element2))
...
def exercise4():
    ...
    tag = "a"
    content = "ClickMe"
    kwargs = {"href": "https://google.com", "_class": "myClass"}
...
def exercise7():
    ...
    def solution1(*args):
        to_return = dict()
        combos = [(s1, s2) for s1 in args for s2 in args if s1 != s2]
    ...
    print(solution1({1, 2, 3}, {2, 4}, {6, 5, 7, 2}))
...
def exercise8():
    ...
    :return:
    """
    mapping = {'start': 'a', 'b': 'a', 'a': '6', '6': 'z', 'x': '2', 'z': '2',
               '2': '2', 'y': 'start'}
...
def exercise9():
    ...
    def solution1(*args, **kwargs):
        return len([1 for i in args if i in kwargs.values()])

    print(solution1(1, 2, 3, 4, x=1, y=2, z=1, w=5))
